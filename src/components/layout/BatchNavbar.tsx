...
const BatchNavbar = () => {
  ...
  // Determine if navbar is over a dark section (hero on homepage when not scrolled)
    ...
    <>
      <motion.nav
        ...
        className={`fixed top-0 left-0 right-0 z-40 transition-colors duration-500 ${
          scrolled
            ? "bg-tempo-bone/90 backdrop-blur-md border-b border-tempo-vapor"
            : "bg-transparent"
        }`}
      >
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-12">
          <div className="flex items-center justify-between h-14 lg:h-20">
            ...
            <Link
              to="/"
              className={`text-lg lg:text-xl font-black tracking-[0.2em] uppercase transition-colors ${textColor}`}
            >
              TEMPO
            </Link>
        ...
        {mobileOpen && (
          <motion.div
            ...
            className={`md:hidden border-t ${
              scrolled
                ? "bg-tempo-bone/95 backdrop-blur-md border-tempo-vapor"
